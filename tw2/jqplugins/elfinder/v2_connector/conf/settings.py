...
from ..utils.accesscontrol import fs_standard_access
from ..volumes.filesystem import LocalFileSystemVolumeDriver
...
ELFINDER_CONNECTOR_OPTION_SETS = {
    ...
    'default' : {
        ...
        'roots' : [
            ...
            {
                'id' : 'lff',
                'driver' : LocalFileSystemVolumeDriver,
                'path' : join(MEDIA_ROOT, u'files'),
                'alias' : 'Elfinder files',
                #open this path on initial request instead of root path
                #'startPath' : '',
                'URL' : '%sfiles/' % MEDIA_URL,
    ...
    #option set to only allow image files -- not sure this will be used...
    'image' : {
        'debug' : False,
        'roots' : [
            {
...
# see django.conf.__init__.settings (LazySettings)
# this is all environment/variable settings which it'd be
# tricky to fully cover...
